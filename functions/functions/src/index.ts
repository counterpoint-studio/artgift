...
let messageTemplates: {
  [locale: string]: { [key: string]: Handlebars.TemplateDelegate };
} | null = null;
  ...
  if (!messageTemplates) {
    ...
    for (let locale of Object.keys(messageTemplateSources)) {
      ...
      for (let msg of Object.keys(messageTemplateSources[locale])) {
        messageTemplates[locale][msg] = Handlebars.compile(
          messageTemplateSources[locale][msg]
        );
      }
    }
  }
...
export const makeSlotsAvailableBasedOnAppState = functions
  .region("europe-west1")
  .firestore.document("appstates/{appStateId}")
  .onWrite(async (change) => {
    if (!change.after.exists) return;
    let appState = change.after.data()!.state;
    if (appState === "open") {
      let slotsToUpdate = await db
        .collection("slots")
...
export const processSlotReservation = functions
  ...
  .onCreate((document) => {
    ...
    return db.runTransaction(async (tx) => {
      ...
      let [slot, prevGift] = await Promise.all([
        tx.get(slotRef),
        tx.get(giftRef),
      ]);

      let prevSlotRef =
        prevGift.exists && prevGift.data()!.slotId
          ? db.collection("slots").doc(prevGift.data()!.slotId)
          : undefined;
      ...
      let slotAvailable =
        slot.exists &&
        (slot.data()!.status === "available" || prevSlot?.id === slotId);
      if (slotAvailable) {
        await tx.set(
          giftRef,
          {
...
export const releaseSlotOnGiftDelete = functions
  .region("europe-west1")
  .firestore.document("gifts/{giftId}")
  .onDelete(async (snap) => {
    let statusBefore = snap.data()!.status;
    if (
      statusBefore &&
      (statusBefore === "creating" ||
  ...
  .onUpdate(async (change) => {
    ...
    if (
      ...
      (statusBefore === "creating" ||
        ...
        statusBefore === "confirmed") &&
      statusAfter &&
      (statusAfter === "rejected" || statusAfter === "cancelled") &&
...
export const createGiftMessage = functions
  ...
  .onWrite(async (change, context) => {
    ...
    if (
      ...
      previousDocument.status === "creating"
    ) {
      ...
      previousDocument.status !== "confirmed"
    ) {
      ...
      previousDocument.status !== "rejected"
    ) {
      ...
      previousDocument.status !== "cancelled"
    ) {
...
export const createGiftReminderMessages = functions
  .region("europe-west1")
  .pubsub.schedule("every 1 hours")
  .onRun(async () => {
    let confirmedGifts = await db
      .collection("gifts")
    ...
    for (let gift of confirmedGifts.docs) {
      ...
      let { date, time } = await (
        await db.collection("slots").doc(giftData.slotId).get()
      ).data()!;
      let giftDate = parseDateTime(date, time);
      let giftTimestamp =
        giftDate.getTime() - TIME_ZONE_UTC_OFFSET * 60 * 60 * 1000;
      let timeFromNow = giftTimestamp - Date.now();
      if (timeFromNow < REMINDER_PERIOD && timeFromNow > REMINDER_PERIOD / 2) {
        let giftMessages = await db
          .collection("messages")
          .where("giftId", "==", gift.id)
          .get();
        let existingReminder = giftMessages.docs.find(
          (d) => d.data().messageKey === "giftReminder"
        );
        if (!existingReminder) {
          let emailSubject = getMessageTemplates()[
            giftData.fromLanguage || "en"
          ].giftReminderEmailSubject({});
          let emailBodyTemplate = getMessageTemplates()[
            giftData.fromLanguage || "en"
          ].giftReminderEmailBody;
          let smsTemplate = getMessageTemplates()[giftData.fromLanguage || "en"]
            .giftReminderSMS;
...
export const populateArtistItinerariesOnArtistUpdate = functions
  .region("europe-west1")
  .firestore.document("artists/{artistId}")
  .onWrite(async (change) => {
    let appState = (
      await db.collection("appstates").doc("singleton").get()
    ).data();
    ...
    let itinerariesBefore = change.before.exists
      ? sortBy(
          change.before
            .data()!
            .itineraries.map((it: any) => pick(it, "region", "from", "to")),
          (it) => it.from.date,
          (it) => it.from.time
        )
      : [];
    ...
    console.log(
      "on artist update, populating itineraries for regions",
      Array.from(affectedRegions)
    );
    for (let region of Array.from(affectedRegions)) {
      await backOff(() =>
        db.runTransaction((tx) => populateArtistItineraries(region, tx))
      );
    }
  });
...
export const sendArtistInvitation = functions
  ...
  .onUpdate(async ({ before, after }) => {
    let beforeInvitationTrigger =
      before.exists && before.data()!.invitationTrigger;
    let afterInvitationTrigger =
      after.exists && after.data()!.invitationTrigger;
    if (
      afterInvitationTrigger &&
      afterInvitationTrigger !== beforeInvitationTrigger
    ) {
      ...
      if (phoneNumber || email) {
        let emailSubject = getMessageTemplates()[
          DEFAULT_LANGUAGE
        ].artistCreatedEmailSubject({});
        let emailBodyTemplate = getMessageTemplates()[DEFAULT_LANGUAGE]
          .artistCreatedEmailBody;
        let smsTemplate = getMessageTemplates()[DEFAULT_LANGUAGE]
          .artistCreatedSMS;
        let baseUrl = functions.config().artgift.baseurl;
...
export const populateArtistItinerariesOnGiftUpdate = functions
  ...
  .onWrite(async (change) => {
    ...
    let involvesConfirmedGift =
      change.before.data()?.status === "confirmed" ||
      change.after.data()?.status === "confirmed";
    ...
    console.log(
      "on gift update, populating itineraries for regions",
      Array.from(affectedRegions),
      "based on slots",
      Array.from(affectedSlotIds)
    );
    for (let region of Array.from(affectedRegions)) {
      await backOff(() =>
        db.runTransaction((tx) => populateArtistItineraries(region, tx))
...
export const scheduledFirestoreBackup = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    let databaseName = client.databasePath(projectId, "(default)");
...
async function populateArtistItineraries(
  region: string,
  ...
  let affectedArtists =
    affectedArtistRefs.length > 0 ? await tx.getAll(...affectedArtistRefs) : [];
  let affectedSlots =
    affectedSlotRefs.length > 0 ? await tx.getAll(...affectedSlotRefs) : [];
  let affectedGifts =
    affectedGiftRefs.length > 0 ? await tx.getAll(...affectedGiftRefs) : [];

  console.log(
    "populating",
    affectedGifts.length,
    "gifts from",
    slots.docs.length,
    " slots amongst",
    affectedArtists.length,
    "artists in ",
    region
  );
  ...
  let slotsInTimeOrder = sortBy(
    affectedSlots
      .filter(
        (slot) =>
          !!affectedGifts.find((g) => (g.data() as any).slotId === slot.id)
      )
      ...
      data: {
        ...
        itineraries: sortBy(
          (a.data() as any).itineraries.map((i: any) => ({
            ...i,
            assignments:
              i.region === region || !i.assignments ? [] : i.assignments, // clear existing assignments for this region
          })),
          (i) => i.from.date,
          (i) => i.from.time
        ),
  ...
  for (let slot of slotsInTimeOrder) {
    ...
    let bestArtistIdx = -1,
      bestItineraryIdx = -1,
      bestItineraryGapSincePrevious = 0;
    for (let i = 0; i < artistData.length; i++) {
      for (let j = 0; j < artistData[i].data.itineraries.length; j++) {
        ...
        if (
          slotDate.getTime() >= itFrom.getTime() &&
          slotDate.getTime() < itTo.getTime()
        ) {
          let lastSlotId = getLastAssignedSlotId(artistData[i].data, region);
          let lastSlot = lastSlotId
            ? slotsInTimeOrder.find((s) => s.id === lastSlotId)?.data
            : null;
          let gapSinceLastSlot = lastSlot
            ? slotDate.getTime() -
              parseDateTime(lastSlot.date, lastSlot.time).getTime()
            : Number.MAX_VALUE;
    ...
    if (bestArtistIdx >= 0 && bestItineraryIdx >= 0) {
      artistData[bestArtistIdx].data.itineraries[
        bestItineraryIdx
      ].assignments.push({ slotId: slot.id, giftId: gift.id });
    }
  }
...
function getLastAssignedSlotId(artistData: any, region: string) {
  for (let i = artistData.itineraries.length - 1; i >= 0; i--) {
    if (
      artistData.itineraries[i].region === region &&
      artistData.itineraries[i].assignments.length > 0
    ) {
...
async function createMessage(
  giftId: string,
  giftDocument: FirebaseFirestore.DocumentData,
  messageKey: string,
  eventId: string
) {
  ...
  if (await shouldCreate(messageRef)) {
    let slot = await (
      await db.collection("slots").doc(giftDocument.slotId).get()
    ).data();
    let emailBodyTemplate = getMessageTemplates()[
      giftDocument.fromLanguage || "en"
    ][messageKey + "EmailBody"];
    let smsTemplate = getMessageTemplates()[giftDocument.fromLanguage || "en"][
      messageKey + "SMS"
    ];
    let emailSubject = getMessageTemplates()[giftDocument.fromLanguage || "en"][
      messageKey + "EmailSubject"
    ]({});
    let baseUrl = functions.config().artgift.baseurl;
...
export const sendMessages = functions
  .region("europe-west1")
  .pubsub.schedule("every 1 minutes")
  .onRun(async () => {
    ...
    unsentMessages.forEach(async (doc) => {
      ...
      let smsSend =
        toNumber && toNumber !== NOOP_PHONE_NUMBER
          ? sendSMS(smsBody, toNumber)
          : Promise.resolve();
      let emailSend = toEmail
        ? sendEmail(emailSubject, emailBody, toEmail, toName)
        : Promise.resolve();
      await Promise.all([smsSend, emailSend]);
      doc.ref.set(
        { sent: true, sentAt: admin.firestore.FieldValue.serverTimestamp() },
        { merge: true }
      );
    });
  });
